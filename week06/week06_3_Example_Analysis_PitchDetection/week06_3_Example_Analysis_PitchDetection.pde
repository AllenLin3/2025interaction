...
SoundFile sound;// 新加這一行, 因為要把beat.mp3 讀進來
...
void setup() {
  size(640, 320);
  background(0);
  sound = new SoundFile( this, "beat.mp3");// 新加這一行,把beat.mp3 讀進來
  sound.loop();
  pitchDetector = new PitchDetector(this);//新增、初始化PitchDetector 物件
  // capture audio input
  //pitchDetector.input(new AudioIn(this, 0));原本的程式, 是用 Audio in 音效卡的音源輸入
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0],pitchAndConfidence[1]);//把pitch和confidence印出來
  }
